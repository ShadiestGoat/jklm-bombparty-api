...
func bToID(raw json.RawMessage) string {
	return string(raw)
}
...
func (c *Client) eventHandle(data []byte) {
	...
	case CHATTER_JOINED:
		// 42["chatterAdded",{"nickname":"pekopeko"}]
		c.Room.lastChatterID++
		chatter := rawProfile{}
		tryUnmarshal(out[1], &chatter)
	...
	case PLAYER_JOINED_ROUND:
		...
		sendEv(c, PLAYER_JOINED_ROUND, &EventPlayerJoinedRound{
			Player: p,
		})
	case PLAYER_LEFT_ROUND:
		// 42["removePlayer",20]
		id := string(out[1])
		delete(c.Room.PlayersWaiting, id)
		sendEv(c, PLAYER_LEFT_ROUND, &EventPlayerLeftRound{
			Player: c.Room.Chatters[string(data)],
		})
	case COUNTDOWN:
		// 42["setStartTime",1673345066462,1673345051462]
		// start time, current Time
		rawT := int64(0)
		tryUnmarshal(out[1], &rawT)
	...
	case PLAYER_INPUT:
		// 42["setPlayerWord",54,"j"]
		playedID := bToID(out[1])
		guess := ""
		tryUnmarshal(out[2], &guess)

		player := c.Room.Round.Players[playedID]
		player.Guess = guess

		sendEv(c, PLAYER_INPUT, &EventPlayerInput{
			RoundPlayer: player,
		})
	case WORD_CORRECT:
		raw := rawWordCorrect{}
